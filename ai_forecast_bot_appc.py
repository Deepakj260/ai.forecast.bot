...
st.title("📈 AI Forecast Bot with Adjustment Logic")

hist_file = st.file_uploader("📄 Upload 2-Year History File (Firm + Lifting)", type="xlsx")
adjustment_file = st.file_uploader("📄 Upload April 2025 Adjustment File", type="xlsx")
...
if hist_file:
    ...
    adjustment_dict = {}
    if adjustment_file:
        adj_df = pd.read_excel(adjustment_file)
        if 'Part No' in adj_df.columns and 'Forecasted' in adj_df.columns and 'Actual' in adj_df.columns:
            adj_df['Adjustment'] = adj_df.apply(lambda r: (r['Actual'] / r['Forecasted']) if r['Forecasted'] else 1.0, axis=1)
            adjustment_dict = dict(zip(adj_df['Part No'], adj_df['Adjustment']))
    ...
    for part_no, group in df.groupby('Part No'):
        ...
        adjust_factor = adjustment_dict.get(part_no, 1.0)
